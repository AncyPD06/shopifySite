...
theme.Instagrams = (function() {
  function Instagrams(container) {
    ...
    this.settings = {
      ...
      slidesToShow992 	: this.$container.data('slidestoshow-tablet') || null,
      slidesToShow480 	: this.$container.data('slidestoshow-mobile') || null,
    }
    ...
    if(!this.settings.slidesToShow992){
      this.settings.slidesToShow992  		= (this.settings.slidesToShow - 2) > 1? (this.settings.slidesToShow - 2) : 1;
    }
    if(!this.settings.slidesToShow480){
      this.settings.slidesToShow480  		= 2;
    }
...
theme.slickCarousel = (function (){
  function Carousels(container) {
    ...
    this.settings = {
      ...
      autoplaySpeed 	: this.$container.data('speed') || 3000,
      draggable 		: this.$container.data('draggable') || false,
      accessibility 	: this.$container.data('accessibility') || true,
      asNavFor          : this.$container.data('asnavfor') || null,
      slidesToShow992 	: this.$container.data('slidestoshow-tablet') || null,
      slidesToShow480 	: this.$container.data('slidestoshow-mobile') || null,
    }
    console.log("SLick", JSON.stringify(this.settings));
    this.settings.slidesToShow1200 		= (this.settings.slidesToShow - 1) > 1? (this.settings.slidesToShow - 1) : 1;
    if(!this.settings.slidesToShow992){
      this.settings.slidesToShow992  		= (this.settings.slidesToShow - 2) > 1? (this.settings.slidesToShow - 2) : 1;
    }
...
theme.exLink = (function(){
  ...
  for (let i = 0, linksLength = links.length ; i < linksLength ; i++) {
    if ([window.location.hostname, 'grtr.gd'].includes(links[i].hostname) === false && $(links[i]).attr('href') !== 'javascript:void(0)') {
